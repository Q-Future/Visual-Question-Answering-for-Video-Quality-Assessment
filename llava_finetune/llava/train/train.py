...
class ModelArguments:
    ...
    unfreeze_language_model: bool = field(default=True)
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] and  "frames per sec" not in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if len(image_file) > 1:
                    ...
                    image = [([[im[0]],[im[0].repeat(4)]], im[1], "image") for im in image]
            else:
                image = self.process_image(image_file)
                image = [([[image[0]],[image[0].repeat(4)]], image[1], "image")]
                # print(self.process_image(image_file))
            sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)

        elif "frames per sec" in sources[0]:
            if 'B054' in self.list_data_dict[i]["image"]:
                video_file = self.list_data_dict[0]["image"]
            else:
                video_file = self.list_data_dict[i]["image"]
    ...
    #        try:
            if "shareVideoGPTV" in video_file:
                    ...
                    video,frame_idx = process_video_with_decord(video_file, self.list_data_dict[i]["frames per sec"])

                    processor = self.data_args.image_processor
                    image = processor.preprocess(video, return_tensors="pt")["pixel_values"]
                    #modality标签为video，同时输入视频帧数截断至4的整数倍的视频帧用于适配slowfast以及视频每秒一帧的关键帧用于适配siglip
                    image = [([[image[:image.shape[0]//4*4]], [image[frame_idx]]], video[0].size, "video")]
                    if "gt_score" in self.list_data_dict[i]:
                        sources1 = copy.deepcopy(torch.FloatTensor([float(e["gt_score"]) for e in sources]))
                    sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
#        sources1 = copy.deepcopy(torch.FloatTensor([float(e["gt_score"]) for e in sources]))
#        except Exception as e:
#                print(f"Error: {e}")
...
#        data_dict["mos"]=sources1
        ...
        data_dict["task_mark"]=self.list_data_dict[i].get("task_mark", i)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        if "mos" in instances[0]:
            #用于打分任务设计MOS本身的loss(如r1以及r2 loss等）,这里并未使用
            input_ids, labels,mos = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels","mos"))
        # input_ids, labels, ids = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "id"))
        else:
             #用于区分任务类型设计，这里并未使用task_mark，只是将其作为参数传入
            input_ids, labels,task_mark= tuple([instance[key] for instance in instances] for key in ("input_ids", "labels","task_mark"))
        ...
        if "mos" in instances[0]:
            batch = dict(input_ids=input_ids, labels=labels.long() if labels.dtype == torch.int32 else labels, attention_mask=input_ids.ne(self.tokenizer.pad_token_id),mos=mos[0])
        else:
            #正常应该运行到这里
            batch = dict(input_ids=input_ids, labels=labels.long() if labels.dtype == torch.int32 else labels,
                         attention_mask=input_ids.ne(self.tokenizer.pad_token_id),task_mark=task_mark[0])
...
#        if "mos" in instances[0]:
#            print("yes")
#            batch["mos"] = [instance["mos"] for instance in instances]
...
def train(attn_implementation=None):
    ...
    # for name,p in model.get_model().slowfast.named_parameters():
    #     if p.numel() == 0:
    #         print(1)
    #         print(p)
    #         print(name)
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            model.get_model().slowfast.requires_grad_(False)
            model.get_model().slowfast_projector.requires_grad_(False)
            ...
            if "mm_slowfast" in tunable_parts:
                for p in model.get_model().slowfast.parameters():
                    # if p.numel() == 0:
                    #     print(p)
                    # p[torch.isnan(p)] = 0
                    p.requires_grad = True
            if "mm_slowfast_projector" in tunable_parts:
                for p in model.get_model().slowfast_projector.parameters():
                    p.requires_grad = True

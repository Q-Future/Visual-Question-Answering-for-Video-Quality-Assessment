...
import sys
sys.path.append("/tos-bjml-researcheval/jiaziheng/AIGV/qwen2-5_finetune/src")
print(sys.path)
...
def configure_motion(model, training_args, compute_dtype, device):
    motion_extractor = model.slowfast
    motion_extractor.to(dtype=compute_dtype, device=device)
    
    motion_projector =model.slowfast_projector
    motion_projector.to(dtype=compute_dtype, device=device)
    slowfast_params = model.slowfast.parameters()
    set_requires_grad(slowfast_params, not training_args.freeze_motion)
    motion_projector_params = model.slowfast_projector.parameters()
    set_requires_grad(motion_projector_params, not training_args.freeze_motion)
...
def train():
    ...
    if "Qwen2.5" in model_args.model_id:
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            ...
            attn_implementation=None if not training_args.disable_flash_attn2 else "sdpa", 
            **bnb_model_from_pretrained_args
        )
    else:
    ...
    configure_motion(model_to_configure, training_args, compute_dtype, training_args.device)

        ...
        for _, j in enumerate(range(0, len(sources), 2)):
            ...
            if DEFAULT_IMAGE_TOKEN in user_input:
                ...
                if "Qwen2.5" in self.model_id:
                    inputs = processor(text=[user_input], images=images, videos=[videos[0][0]],video_motion=videos[0][1], padding=False, return_tensors='pt', **video_kwargs)
                    all_second_gird.extend(inputs["second_per_grid_ts"])
                else:
                    inputs = processor(text=[user_input], images=images, videos=videos, padding=False, return_tensors='pt')
                prompt_input_ids = inputs['input_ids']
                all_pixel_values.append([inputs[pixel_key],videos[0][1]])
                # all_pixel_values.append(videos[0][1])
                all_image_grid_thw.append(inputs[grid_key])
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            if "pixel_values_videos" in keys:
                batch_pixel_video_values.append(example["pixel_values_videos"][0])
                batch_pixel_video_motion.append(example["pixel_values_videos"][1])
        ...
        if len(batch_pixel_video_values) > 0:
            pixel_video_values = [torch.cat(batch_pixel_video_values,0),torch.cat(batch_pixel_video_motion,0)]
...
def llava_to_openai(conversations, is_video=False):
    ...
    for conversation in conversations:
        transformed_content = replace_image_tokens(conversation["content"], is_video=is_video)
        transformed_entry = {
